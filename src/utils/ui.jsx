...
export const formatDateIndonesia = (inputDate) => {
  ...
  const bahasaDays = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
  const bahasaMonths = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];

  const formattedBahasaDate = formattedDate.replace(new RegExp(options.weekday, 'g'), bahasaDays[date.getDay()]).replace(new RegExp(options.month, 'g'), bahasaMonths[date.getMonth()]);
...
export const ieeeCitationFormatter = (authors_list, title, journal_name, city, volume, issue, pageFrom, pageTo, months, years, publihser, doiStr) => {
...
export const ieeeCitationFormatBooks = (authorsList, title, edition, city, publisher, years) => {
...
export const ieeeCitationFormatBookSection = (authorsList, bookSection, title, bookEdition, publisher, years, volumeChapter, pageFrom, pageTo) => {
...
export const ieeeCitationFormatProceedings = (authorsList, title, proceedingTitle, city, years, pageFrom, pageTo, doiStr) => {
...
export const ellipsisGenerator = (descriptions) => {
  return {
    rows: 2,
    expandable: true,
    symbol: 'more',
